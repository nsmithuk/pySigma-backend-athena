...
class athenaBackend(athenaBaseBackend):
    ...
    event_count_condition_expression: ClassVar[Optional[Dict[str, str]]] = {
        "default": "SELECT * FROM event_counts WHERE event_count {op} {count}",
    }
    ...
    def convert_correlation_rule_from_template(
        ...
        # 'WITH event_counts AS (SELECT {selectfields}, COUNT(*) OVER ({aggregate}) as event_count FROM {table} WHERE {search} ORDER BY {timefield}) {condition}'
    ...
    def convert_correlation_search(
        ...
        **kwargs,
        ...
        raise NotImplementedError(
            f"Multiple rules are not supported by backend."
        )
